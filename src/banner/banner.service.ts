...
import { Banner, BannerDocument, BannerType, BannerPosition } from './schema/banner.schema';
...
export class BannersService {
  ...
  async create(createBannerDto: CreateBannerDto, createdById: string): Promise<BannerDocument> {
  ...
  async findPublicBanners(type?: BannerType, position?: BannerPosition, userAgent?: string): Promise<any> {
    const now = new Date();
    ...
    this.trackViews(banners.map(b => b._id), userAgent);
  ...
  async update(id: string, updateBannerDto: UpdateBannerDto, updatedById: string): Promise<BannerDocument> {
    ...
    if (updateBannerDto.linkType && updateBannerDto.targetUrl) {
      this.validateTargetUrl(updateBannerDto.linkType, updateBannerDto.targetUrl);
    }
  ...
  private async trackViews(bannerIds: string[], userAgent?: string): Promise<void> {
    try {
  ...
  async toggleStatus(id: string, updatedById: string): Promise<any> {
    ...
    return {
      message: banner.isActive ? 'Đã bật hiển thị banner' : 'Đã tắt hiển thị banner',
      isActive: banner.isActive
    };
  }

  async updateOrder(id: string, order: number, updatedById: string): Promise<BannerDocument> {
    const banner = await this.bannerModel.findByIdAndUpdate(
      id,
      { 
        order,
  ...
  async bulkUpdateOrder(updates: Array<{ id: string; order: number }>, updatedById: string): Promise<any> {
  ...
  async getAnalytics(startDate?: string, endDate?: string): Promise<any> {
    ...
    const avgClickRate = stats.totalViews > 0 
  ...
  async getBannerAnalytics(bannerId: string, period: string = '30d'): Promise<any> {
    const banner = await this.bannerModel.findById(bannerId).select('title type position clickCount viewCount');
    if (!banner) {
      throw new NotFoundException('Không tìm thấy banner');
    }
    ...
    const clickRate = banner.viewCount > 0 
      ? Math.round((banner.clickCount / banner.viewCount) * 10000) / 100 
      : 0;

    // TODO: Implement daily stats if you have tracking collection
    // For now, return mock daily data
    const dailyStats = this.generateMockDailyStats(days, banner.clickCount, banner.viewCount);
    ...
    switch (linkType) {
      case 'web':
        if (!targetUrl.match(/^https?:\/\/.+/)) {
          throw new BadRequestException('Web link phải bắt đầu bằng http:// hoặc https://');
        }
        break;
      case 'deeplink':
        if (!targetUrl.match(/^[a-zA-Z][a-zA-Z0-9+.-]*:\/\/.+/)) {
          throw new BadRequestException('Deep link không đúng định dạng (vd: myapp://...)');
        }
        break;
      case 'internal':
  ...
  private generateMockDailyStats(days: number, totalClicks: number, totalViews: number): any[] {
    const stats = [];
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const clicks = Math.max(0, avgDailyClicks + Math.round((Math.random() - 0.5) * avgDailyClicks * 0.5));
      const views = Math.max(0, avgDailyViews + Math.round((Math.random() - 0.5) * avgDailyViews * 0.3));
