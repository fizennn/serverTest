...
import { ReturnOrderDto, UpdateReturnStatusDto, PaginatedReturnOrderResponseDto } from '../dtos/return-order.dto';
...
export class ReturnOrdersController {
  ...
  @ApiOperation({
    ...
    description: 'Khách hàng tạo yêu cầu trả hàng cho đơn hàng đã giao thành công. Chỉ có thể trả hàng trong vòng 7 ngày sau khi giao hàng thành công.'
  })
  ...
  @ApiResponse({ status: 403, description: 'Không có quyền trả hàng đơn hàng này' })
  ...
  async createReturnRequest(
    ...
    return this.returnOrdersService.createReturnRequest(orderId, returnOrderDto, user._id.toString());
  }
  ...
  async getMyReturnRequests(
    ...
    return this.returnOrdersService.getCustomerReturnRequests(user._id.toString(), page, limit);
  }
  ...
  @ApiOperation({
    ...
    description: 'Admin cập nhật trạng thái yêu cầu trả hàng và xử lý hoàn tiền'
  })
  @ApiParam({ name: 'id', description: 'ID yêu cầu trả hàng' })
  ...
  async updateReturnStatus(
    ...
    return this.returnOrdersService.updateReturnStatus(id, updateReturnStatusDto);
  }
  ...
  @ApiOperation({
    ...
    description: 'Admin xóa yêu cầu trả hàng (chỉ cho phép xóa những yêu cầu đang chờ xử lý)'
  })
  @ApiParam({ name: 'id', description: 'ID yêu cầu trả hàng' })
  ...
  @ApiQuery({ name: 'startDate', required: false, description: 'Ngày bắt đầu (YYYY-MM-DD)', example: '2025-01-01' })
  @ApiQuery({ name: 'endDate', required: false, description: 'Ngày kết thúc (YYYY-MM-DD)', example: '2025-12-31' })
